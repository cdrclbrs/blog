...
un OTP peut être volé en temps réel via un proxy de phishing type **EvilGinx**, un SMS peut être détourné (SIM-swap), et les « push » peuvent être approuvés par des attaques de **MFA fatigue** 
...
Quand plusieurs applications doivent reconnaître le même user, on parle de SSO. La federation d'identité ca a plus de 20 ans ce n'est pas nouveau, mais c'est important de bien comprendre le fonctionnement car ce n'est pas toujours limpide pour pas mal de monde même dans le domaine IT.

La fédération d'identités c'est donc un service central d'Identity provider (IdP) qui authentifie l’utilisateur et émet une assertion ou un jeton que les applications (services providers) acceptent.
...
**OpenID Connect (OIDC)** vient compléter OAuth : il ajoute la couche du “qui suis-je ?” via un ID Token, une preuve standardisée de l’identité de l’utilisateur.
...
Le code expire vite, ce qui est intuitif pour l’utilisateur. 
Il suffit d’une horloge à peu près correcte. Mais on dépend toujours d’un secret partagé et le phishing en temps réel reste possible : tapez le code au mauvais endroit come sur le site de l'attaquant, et quelqu’un peut le relayer instantanément.
...
Puis, l’authentificateur signe le challenge, lié à l’origin. Le serveur vérifie et c’est terminé. 

Pas de mot de passe, pas d’OTP, pas de « code reçu par SMS ». C’est tres rapide. 

### Pourquoi c’est vraiment résistant au phishing ?

- La signature ne vaut que pour le domaine exact.
- Le faux site « pawn.me » n’obtiendra jamais une preuve valable pour « secure.com ». 
- Et comme il n’y a pas de secret serveur à voler, les fuites massives de passwords ne sont plus un problème. WIN-WIN
